...
abstract class UfPdf extends FpdiRotateCode128
{
    ...
    function Text($x, $y, $txt, $fstroke=0, $fclip=false, $ffill=true, $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M', $rtloff=false)
    {
        //Output a string
        $s=sprintf('BT %.2f %.2f Td %s Tj ET',$x*$this->k,($this->h-$y)*$this->k,$this->_escapetext($txt));
        if($this->underline and $txt!='')
            $s.=' '.$this->_dounderlineWidth($x,$y,$this->GetStringWidth($txt),$txt);
        if($this->ColorFlag)
    ...
    function Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M')
    {
        ...
        if($txt!='')
        {
            ...
            if($align=='R')
                $dx=$w-$this->cMargin-$width;//todo: tpdf
            elseif($align=='C')
                $dx=($w-$width)/2;
            else
                $dx=$this->cMargin;//todo: tpdf
            if($this->ColorFlag)
                $s.='q '.$this->TextColor.' ';
            $txtstring=$this->_escapetext($txt);
            $s.=sprintf('BT %.2f %.2f Td %s Tj ET',($this->x+$dx)*$k,($this->h-($this->y+.5*$h+.3*$this->FontSize))*$k,$txtstring);
            if($this->underline)
                $s.=' '.$this->_dounderlineWidth($this->x+$dx,$this->y+.5*$h+.3*$this->FontSize,$width,$txt);
            if($this->ColorFlag)
                $s.=' Q';
            if($link)
    ...
    function _dounderlineWidth($x,$y,$width,$txt)
    {
        //Underline text
...
// Correctly handles all illegal UTF-8 sequences.
    ...
    function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false, $ln=1, $x=null, $y=null, $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0, $valign='T', $fitcell=false)
    {
        ...
        foreach($codepoints as $k => $cp)
        {
            ...
            if($char=="\n")
            {
                ...
                $this->Cell($w,$h,substr($s,$j,$i-$j),$b,2,$align,$fill, $ln);
                $i++;
                $sep=-1;
                $j=$i;
                $l=0;
            ...
            if($l>$wmax - 1000)
            {
                ...
                else
                {
                    ...
                    $this->Cell($w,$h,substr($s,$j,$sep-$j),$b,2,$align,$fill, $ln);
                    $j=$sep + 1;
                    $i += $lens[$k];
                }
                $sep=-1;
